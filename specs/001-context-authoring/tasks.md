...
- [~] T050 [P] Ejecutar validación quickstart.md (S1–S5) end-to-end — **pasada ejecutada el 2026-08-23** con `llama.cpp server` + `qwen2.5-7b-q4km`, tres corridas (arnés `live_backend.rs`). S1, S2, S3 y S5 ejercitados; **cinco hallazgos** documentados en `quickstart.md` e issues #23, #24, #25. **Queda S4 sin ejercitar**: el agente nunca intenta la URL privada, y decidir si debería es la pregunta abierta de #25
