...
- [ ] T056 Streaming token a token (`agent.token`) durante el refinamiento — **dentro de v1** ([#44](https://github.com/jorelcb/codify/issues/44)). **Requiere decidir antes** si el turno deja de bloquear: hoy `submit_message` retorna cuando el turno está resuelto, y con un modelo local grande eso son minutos (~6 tokens/s medidos con `Qwen2.5-32B`). No es añadir un evento — cambia el contrato del comando y la máquina de estados de la interfaz. Se queda en este spec, y no en uno propio, porque el streaming **es** la experiencia de authoring; lo que le faltaba era trazabilidad, no un spec
